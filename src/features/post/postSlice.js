...
const slice = createSlice({
  ...
  reducers: {
    ...
    deletePostSuccess(state, action) {
      ...
      const postIdDelete = action.payload;
      state.currentPagePosts.forEach((postId, index) => {
        if (postId === postIdDelete) {
    ...
    updatePostSuccess(state, action) {
      ...
      const postUpdated = state.postsById[data._id];
      state.postsById[data._id] = {
        ...postUpdated,
...
export const deletePost =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      dispatch(slice.actions.deletePostSuccess(response.data._id));
      toast.success(response.message);
      dispatch(getCurrentUserProfile());
    } catch (error) {
      dispatch(slice.actions.hasError(error.message));
